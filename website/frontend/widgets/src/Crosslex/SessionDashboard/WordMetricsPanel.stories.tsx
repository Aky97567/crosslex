...
const wiredEvents = (key: string): ExerciseEvent[] =>
...
const buildAdapter = (s: State) => {
  const exerciseLog: ExerciseEvent[] = [
    ...
    ...WIRED_POOL.slice(0, s.wiredCount).flatMap(wiredEvents),
  ];
...
export default {
  ...
  argTypes: {
    ...
    wiredCount: { control: { type: 'range', min: 0, max: WIRED_POOL.length }, name: 'Wired' },
...
export const InfernoBadge = snapshot({
  seedCount: 4, familiarCount: 3, wiredCount: 4,
...
export const CenturyBadge = snapshot({
  seedCount: 2, familiarCount: 5, wiredCount: 5,
