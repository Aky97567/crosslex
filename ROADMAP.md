...
- `[P1]` **Test coverage beyond the session core** — `sessionAlgorithm.ts` (pickNextCard, generateExerciseData/generateExerciseDataSafe) and `sessionStorage.ts`'s streak logic and readiness computation now have tests — 97.6%/92.85% stmt/branch coverage on the former. All 15 frontend/lib packages that declared a `test` script but had no `jest`/`ts-jest`/`@types/jest`/`jest-environment-jsdom` in their own `devDependencies` (Yarn 4's node-modules linker doesn't resolve the root-level ones) have since had that fixed too, and `yarn test` (`--selectProjects unit` for the backend, to skip its DB-dependent e2e suite) now runs clean across the whole monorepo — CI (`.github/workflows/test.yml`) runs it on every PR, required to merge. Actual test *content* beyond the session core is still zero: every one of those 15 packages passes via `passWithNoTests`, not real assertions. `eslint` has the same missing-devDependency problem in `frontend/features` (and likely the others), discovered but not fixed here.
- `[P3]` **`lib/system/app` and `lib/system/runtime` have no `tsconfig.json`** — discovered while wiring up the repo-wide jest fix above; both have real `.ts` source and a `jest.config.js` (which presets `ts-jest`, itself requiring a tsconfig to compile anything), but no tsconfig exists in either package. Currently inert since neither has test files yet — the moment one is added, `ts-jest` will need a tsconfig to compile it.
