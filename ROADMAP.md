...
## In-flight / Tech debt
- `[P0]` **`similarWords` generation guidelines, then regenerate known offenders** — same shape as the `wordContext` design-principles work (`docs/word-content.md`): write down what makes a good `similarWords` entry (the doc already says "synonyms, not thematically related words," but doesn't cover derivational relatives), then regenerate whichever entries violate it. One known offender so far: `rente.ts` lists `Altersrente` as a synonym of `Rente`, which — same shape as the now-fixed `Steuer`/`Steuererklärung` case — is a word built *from* the target word, not necessarily the same concept as it. Unlike the `wordContext` bugs, this can't break the exercise mechanically (`similarWords` isn't part of the blanking pipeline), so it's a content-accuracy question, not a functional one. No systematic scan has been run yet to find other offenders — `rente` is just the one spotted by chance.
...
- `[P1]` **No test coverage** — Jest is configured but no tests exist; highest priority targets are `sessionAlgorithm.ts` (pickNextCard, generateExerciseData) and `sessionStorage.ts` (streak logic, readiness computation) — these are complex enough to break silently under refactoring, as the (now-fixed) ContextBlank compound-word bug demonstrated
